...
app.post('/', function(req, res) {
  ...
  const blogDetails = {
    ...
    blogContent: req.body.newpost}
  posts.push(blogDetails);
  res.redirect('/');
});

app.get('/posts/:postsName', function(req, res) {
  const requestedTitle = _lowerCase(req.params.postsName);
  

  posts.forEach(function(post) {
    const storedTitle = _lowerCase(post.blogTitle);

    if (requestedTitle === storedTitle) {
      res.render('/post', {content:posts.blogContent, title:posts.blogTitle});
    };
  });
